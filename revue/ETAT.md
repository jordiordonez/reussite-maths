# Relecture mathématique — état d'avancement

Une ligne par fiche. Le rapport détaillé est dans `revue/<CODE>.md`.

Statut : ⬜ à faire · 🔨 en cours · ✅ relue

| Fiche | Titre | Statut | Anomalies corrigées |
|---|---|---|---|
| 1A | Définitions du produit scalaire | ✅ | 3 (notation dans du texte généré) |
| 1B | Applications du produit scalaire | ⬜ | |
| 1C | Géométrie repérée | ⬜ | |
| 1D | Espace : orthogonalité et distances | ⬜ | |
| 2A | Suites : génération et variations | ⬜ | |
| 2B | Suites arithmétiques et géométriques | ⬜ | |
| 2C | Récurrence et limites | ⬜ | |
| 3A | Nombre dérivé et tangente | ⬜ | |
| 3B | Calculer une dérivée | ⬜ | |
| 3C | Variations et extremums | ⬜ | |
| 3D | Dérivée d'une composée | ⬜ | |
| 3E | Convexité | ⬜ | |
| 4A | Fonctions de référence | ⬜ | |
| 4B | Limites de fonctions | ⬜ | |
| 4C | Continuité et valeurs intermédiaires | ⬜ | |
| 5A | Fonction exponentielle | ⬜ | |
| 5B | Fonctions avec exponentielle | ⬜ | |
| 5C | Logarithme népérien | ⬜ | |
| 6A | Calcul vectoriel dans le plan | ⬜ | |
| 6B | Vecteurs de l'espace | ⬜ | |
| 6C | Droites et plans de l'espace | ⬜ | |
