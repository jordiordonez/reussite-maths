...
Rapport : [revue/10A.md](../../revue/10A.md). Les tests extraient le modèle du HTML livré. L’oracle Python calcule la loi par convolution de Bernoulli, indépendamment de la formule utilisant les coefficients binomiaux dans la fiche.

```bash
python3 outils/chapitre9/test_math.py
python3 outils/check_fiche.py chapitres/10_loi_binomiale/10A_loi_binomiale.html
...
python3 outils/build_site.py --only \
  chapitres/10_loi_binomiale/10A_loi_binomiale.html \
