...
def build(code, filename, title, titles):
    ...
    engine = script[1]
    engine = re.sub(r'  function parseNum\(s\) \{.*?\n  \}', '  function parseNum(s) { return Chapter12.parseNum(s); }', engine, count=1, flags=re.S)
    engine = engine.replace('      cur = gen();', '''      var next = gen(), guard = 0;
      ...
      cur = next;
      Chapter12.activeExercises[n - 1] = cur;''')
    ...
    engine = re.sub(r'  var qcmBank = \[.*?\n  \];', '  var qcmBank = Chapter12.qcmBank;', engine, count=1, flags=re.S)
    engine = engine.replace('      qcmMap[i] = { correct:', '      Chapter12.activeQcm[i] = item;\n      qcmMap[i] = { correct:')
    startup = '''  /* ---------- démarrage ---------- */
  Chapter12.activeExercises = [];
  Chapter12.activeQcm = [];
  Chapter12.generators.forEach(function (gen, index) {
    wireExercise(index + 1, gen, function (e, raw) {
      return Chapter12.accepts(e, raw)
  ...
  renderQcm();'''
    ...
    for token in ['Chapter12.activeExercises[n - 1] = cur','Chapter12.activeQcm[i] = item','check(cur, raw)','Chapter12.generators.forEach']:
